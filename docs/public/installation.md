...
| pvc.metadata.annotations                   | object  | no        | `{}`                     | Custom annotations applied to all Persistent Volume Claims (PVCs) created for RabbitMQ and Backup Daemon. When the parameter is omitted or empty, existing PVC annotations managed by the operator are not changed.                                                                                                                                                                    |
